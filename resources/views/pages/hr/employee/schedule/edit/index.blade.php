<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="relative">
            <h1 class="text-2xl md:text-2xl text-slate-800 dark:text-slate-100 font-bold mb-12">Update Schedule</h1>
        </div>

        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">

            <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
                <form action="{{ route('schedule.update', $emp->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                                                ...
                                                <select name="employee_name" id="employee_name"
                                                    ...
                                                    @foreach ($employees as $employee)
                                                        <option value="{{ $employee->id }}"
                                                            {{ $employee->id == $emp->employee->id ? 'selected' : '' }}>
                                            ...
                                            <div class="md:col-span-1">
                                                ...
                                                <select name="shift" id="shift"
                                                    ...
                                                    <option value="Morning"
                                                        {{ $emp->shift == 'Morning' ? 'selected' : '' }}>Morning
                                                    </option>
                                                    <option value="Afternoon"
                                                        {{ $emp->shift == 'Afternoon' ? 'selected' : '' }}>Afternoon
                                                    </option>
                                                    <option value="Night"
                                                        {{ $emp->shift == 'Night' ? 'selected' : '' }}>Night
                                                    </option>
                                                </select>
                                            </div>
                                        </div>
                                ...
                                <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-1">
                                    <div class="md:col-span-1">
                                        <div class="inline-flex items-start" style="float: right;">
                                            <button type="submit"
                                                class="bg-blue-400 hover:bg-primary-500 text-white font-bold py-2 px-4 rounded">Submit</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
